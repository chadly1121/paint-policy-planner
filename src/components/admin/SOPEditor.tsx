...
const SOPEditor = ({ open, onClose, sopKey, systemTitle, systemContent }: SOPEditorProps) => {
  const { t } = useTranslation();
  const { toast } = useToast();
  const { getCompanySOP, upsertCompanySOP, resetSOPToSystem } = useCompanyContent();
  ...
  useEffect(() => {
    if (open) {
      if (companySOP) {
        setTitle(companySOP.title);
        // Remove the footer when editing
        setContent(companySOP.content.replace(COMPLIANCE_FOOTER, ""));
      } else {
        setTitle(systemTitle);
        setContent(systemContent);
      }
    }
  }, [open, companySOP, systemTitle, systemContent]);
  ...
  const handleSave = async () => {
    ...
    const { error } = await upsertCompanySOP(sopKey, title.trim(), contentWithFooter);
    ...
    if (error) {
      ...
      toast({
        title: "SOP saved",
        description: "Your custom SOP has been saved successfully.",
      });
      onClose();
    }
    setSaving(false);
  ...
  const handleReset = async () => {
    setResetting(true);
    const { error } = await resetSOPToSystem(sopKey);
    ...
    if (error) {
      ...
      toast({
        ...
        description: "The SOP has been reset to the original template.",
      });
      setTitle(systemTitle);
      setContent(systemContent);
      onClose();
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isCustomized 
              ...
              : "Create a custom version of this SOP. The system template will remain unchanged."
            }
          </DialogDescription>
        </DialogHeader>
