...
serve(async (req) => {
  ...
  try {
    const { sectionKey, answers, userId } = await req.json();

    console.log(`Processing quiz submission for section: ${sectionKey}, user: ${userId}`);

    if (!sectionKey || !answers || !userId) {
      return new Response(
    ...
    const pointsEarned = passed ? 100 : 0;

    console.log(`Quiz result: ${score}/${totalQuestions}, passed: ${passed}`);
    ...
    if (passed) {
      ...
      if (!alreadyCompleted) {
        ...
        console.log("Section already completed, no additional points awarded");
      }
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        questions: questionsWithAnswers, // Send answers after submission for review
      }),
